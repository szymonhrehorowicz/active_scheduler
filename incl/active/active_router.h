...
{
  public:
    Router_Interface(etl::message_router_id_t id, Object_Interface &owner) : imessage_router(id), m_owner(owner)
    {
    }
...
template <uint32_t Queue_Size, typename T_Derived, typename... T_Message_Types> class Router : public Router_Interface
{
    static_assert(Queue_Size > 0 && Queue_Size <= 256, "Queue size must be reasonable for embedded system");
    static_assert(sizeof...(T_Message_Types) <= 16, "Too many message types may impact stack usage");

  private:
    class Router_Impl : public etl::message_router<Router_Impl, T_Message_Types...>
    {
      public:
        using Base = etl::message_router<Router_Impl, T_Message_Types...>;

        Router_Impl(T_Derived &parent, etl::message_router_id_t id) : Base(id), m_parent(parent)
        {
        }
  ...
  public:
    ...
    Router(etl::message_router_id_t id, Object_Interface &owner)
        : Router_Interface(id, owner), m_router(*static_cast<T_Derived *>(this), id)
    {
    }
